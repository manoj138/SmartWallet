...
import { 
  IoGridOutline, IoLogOutOutline, IoSettingsOutline, 
  IoTrendingUpOutline, IoWalletOutline, IoShieldCheckmarkOutline 
...
const Asidebar = () => {
  ...
  const menuItems = [
    { name: "Dashboard", path: "/dashboard", icon: <IoGridOutline size={20} />, color: "from-purple-500 to-indigo-500" },
    { name: "Incomes", path: "/dashboard/incomes", icon: <IoTrendingUpOutline size={20} />, color: "from-emerald-500 to-teal-500" },
    { name: "Expenses", path: "/dashboard/expenses", icon: <IoWalletOutline size={20} />, color: "from-rose-500 to-orange-500" },
    { name: "Settings", path: "/dashboard/settings", icon: <IoSettingsOutline size={20} />, color: "from-blue-500 to-cyan-500" },
  ];
  ...
  return (
    <aside className="sticky top-0 h-screen w-72 bg-[#050505] border-r border-white/5 flex flex-col p-6 shadow-2xl z-50 overflow-hidden">
      ...
      >
        ...
        <div>
          <h1 className="text-xl font-black tracking-tighter leading-none italic">SmartWallet</h1>
          <p className="text-[8px] font-black text-gray-500 uppercase tracking-[0.3em]">Premium Ledger</p>
        </div>
      </motion.div>
          ...
          >
            ...
            >
              ...
              <span className={`transition-all duration-500 group-hover:scale-110 ${
                isActive(item.path) ? "text-indigo-400" : "group-hover:text-gray-200"
              }`}>
                {item.icon}
              </span>

              <span className={`font-bold tracking-tight text-sm transition-colors ${
              ...
              <div className={`absolute inset-0 rounded-[1.5rem] bg-gradient-to-r ${item.color} opacity-0 group-hover:opacity-[0.03] transition-opacity`} />
            </Link>
          </motion.div>
        ))}
      </nav>
